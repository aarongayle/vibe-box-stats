...
const TeamDashboard = () => {
  ...
  const loadSummary = useCallback(
    async (gameId, { silent = false } = {}) => {
      ...
      try {
        ...
        const data = await fetchGameSummary(gameId, teamId);
        if (data) {
          setSummary(data);
          setOffline(false);
        }
  ...
  return (
    <div className="min-h-screen bg-ink text-zinc-100" style={{ '--accent-color': team.accent }}>
      <main className="mx-auto flex max-w-3xl flex-col gap-10 px-4 py-8">
        <header className="space-y-3">
          ...
          <h1 className="font-sans text-4xl font-semibold text-zinc-100">{team.fullName} box score, distilled.</h1>
