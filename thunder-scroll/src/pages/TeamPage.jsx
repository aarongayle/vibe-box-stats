...
function TeamPage() {
  ...
  const scheduleContent = useMemo(() => {
    ...
    return schedule.map((game) => (
      <GameCard key={game.id} game={game} isActive={game.id === activeGame?.id} onClick={handleGameClick} teamColor={team.color} />
    ));
  }, [loadingSchedule, schedule, activeGame, team.color]);
    ...
    <div className="min-h-screen bg-ink text-zinc-100">
      <main className="mx-auto flex max-w-3xl flex-col gap-10 px-4 py-8">
        <header className="space-y-3">
          ...
          <h1 className="font-sans text-4xl font-semibold text-zinc-100" style={{ color: team.color }}>
            {team.name}, distilled.
          </h1>
          <p className="max-w-2xl text-sm text-zinc-500">
            Hyper-fast, text-forward updates sourced from ESPN&apos;s hidden endpoints. Nothing but data for{' '}
            <span style={{ color: team.color }}>{team.name}</span> fans.
          </p>
